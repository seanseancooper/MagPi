...
class WifiElasticMappingTransformer:
    ...
    def process_workers(self, item, mapTxfmr):

        parser = mapTxfmr(item)
        ...
        signal_data = parser.get_signal_data()  # why is signal data here in a worker method.
    ...
    def process_signals(self, item, mapTxfmr):
    ...
    def push(self, data, mapTxfmr):
        """ method entrypoint for elastic indexing """
        try:
            # this entire choice should be higher, earlier and remove signal_data and deps from this method
            [self.process_workers(item, mapTxfmr) for item in data]
            [self.process_signals(item, mapTxfmr) for item in data]
