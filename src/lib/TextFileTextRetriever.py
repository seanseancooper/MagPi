...
def looper(q):
    """
    Thread function to continuously write data to a queue.
    """
    ...
    #         import time
    #         time.sleep(3)
    #         if hash(scanned) not in hashes:
    #             q.put(scanned)
    #             hashes.append(hash(scanned))
    ...
    try:
        ...
        while process.stdout:
            q.put(str(b''.join(process.stdout.readlines()).decode(encoding='latin-1')))
            print(f'polled {q.get()}')
...
class TextFileTextRetriever(threading.Thread):
    ...
    def scan(self):
        try:
            with open('/src/lib/scanned.xml') as f:
                scanned = f.read()
            return scanned
