...
# sensitive (i.e. T and t are not the same)A single character code
# that represents the command or response message type. Please be
# aware that this code is case sensitive (i.e. T and t are not the same)
msgCode = b'A'
    ...
    def run(self):
        ...
        try:
            if self.config['TEST_FILE']:
                ...
                lines = [line.strip().replace('"', '').replace('  ', '') for line in open(self.config['TEST_FILE'], 'r')]
                ...
                while True:
                    for line in lines[1:-1]:
                        vals = line.split(',')
                        out = dict([(keys[i], vals[i]) for i in range(len(keys))])
                        if FIX_TIME:
                            out['COMP_DATE'] = format(datetime.now(), self.config['DATE_FORMAT'])
                            out['COMP_TIME'] = format(datetime.now(), self.config['TIME_FORMAT'])
                            out['SCAN_DATE'] = format(datetime.now(), self.config['DATE_FORMAT'])
                            out['SCAN_TIME'] = format(datetime.now(), self.config['TIME_FORMAT'])
                ...
                with serial.Serial(self.device, self.rate, parity=self.parity, bytesize=self.bytesize, stopbits=self.stopbits, timeout=1) as ser:
                    ...
                    while True:
                        # self.resp = ser.readline
                        self.resp = ser.read(10)
                        print("GOT: " + str(self.resp))
