...
to output log data as audio.

this is based on pythonjsonlogger
'''
import logging
import json
import re
...
class SpeechEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        ...
        try:
            return super(SpeechEncoder, self).default(obj)
...
class SpeechFormatter(logging.Formatter):
    """
    A custom formatter to format logging records so they can be spoken strings.
    Extra values will be REMOVED if not supported by json default encoder
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.speech_encoder = self._str_to_fn(kwargs.pop("json_encoder", None))
        ...
        if not self.speech_encoder and not self.json_default:
